...
//                .doFinally(_ -> sendLog(exchange, start, traceId));
    ...
    private void sendLog(ServerWebExchange exchange, long start, String traceId) {
        ...
        GatewayApiAccessVo event = GatewayApiAccessVo.builder()
                ...
                .source("SERVICES")
                .queryString(exchange.getRequest()
                        .getQueryParams()
                        .toSingleValueMap()
                        .toString())
                .traceId(traceId)
        ...
        client().post()
                ...
                .bodyValue(event.toDiscord())
                .retrieve()
                .bodyToMono(Void.class)
                .timeout(Duration.ofSeconds(3))
                .doOnSuccess(_ ->
                        log.debug("[DISCORD] Gateway access log sent. traceId={}", traceId)
                )
                .onErrorResume(e -> {
                    log.error("[DISCORD] Failed to send gateway access log. traceId={}, error={}", traceId, e.getMessage());
                    return Mono.empty();
                })
                .subscribe();
    }
    ...
    private WebClient client() {
        ...
        return WebClient.builder().baseUrl(discordWebhookUrl).build();
    }
